...
void get_color(Scene scene, Ray r, int raydepth, double& color1,
    double& color2, double& color3, bool in, bool& res) { // threading
    res = scene.get_color(r, raydepth, color1, color2, color3, std::ref(in));
}
    ...
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            ...
            for (int k = 0; k < rays_pixel; k++){
              ...
              threads[k] = std::thread(&get_color, scene, dir_ray, 100, std::ref(c1[k]),
                                        std::ref(c2[k]), std::ref(c3[k]), false, std::ref(res1[k]));
